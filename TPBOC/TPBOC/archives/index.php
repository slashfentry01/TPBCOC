    ...
    <body id="page-top">
        ...
        <header class="masthead">
            <div class="container d-flex h-100 align-items-center">
                <div class="mx-auto text-center">
                    ...
                    <a class="btn btn-primary js-scroll-trigger" href="#cases">Cases</a>
        ...
        <section id="cases" class="portfolio">
            <div class="container" data-aos="fade-up">
                <div class="section-title"><h2>Solved Cases</h2></div>
                    <div class='row' style='width: 900px; margin-left: 132.5px'>
                        <?php
                            ...
                            if ($conn->connect_errno)
                            {
                                ...
                                $stmt=$conn->prepare("SELECT blockchain FROM archival WHERE investigator=?");
                                $stmt->bind_param("s", $name); //$_SESSION['name']);
                                $res=$stmt->execute();
                                $stmt->store_result();
                                $stmt->bind_result($string);
                                while($stmt->fetch()){
                                    $BC=unserialize($string);
                                    echo "<div class='column' style='width: 425px; height: 575px;'>";
                                        echo "<div style='width: 425px; height: 550px; padding-top: 0.75rem; padding-bottom: 0.75rem; border-style: solid; border-color: black; border-width: 1px' class='container'>";
                                            echo "<img src='".$BC->chain[0]->Ipath."' style='width: 400px; height:400px' class='img-fluid' alt=''>";
                                            echo "<h4>".$BC->chain[0]->title."</h4>";
                                            if (count($BC->chain) > 2) {
                                                echo "<h6>".$BC->chain[0]->description." - ".(count($BC->chain)-1)." Evidences</h6>";
                                            } else {
                                                echo "<h6>".$BC->chain[0]->description." - ".(count($BC->chain)-1)." Evidence</h6>";
                                            }
                                            echo "<div class='portfolio-links'>";
                                                echo "<form action='evidence.php' method='POST'>";
                                                    ...
                                                    echo "<input type='submit' class='btn btn-outline-success' type='submit' value='View Evidences'>";
                                                echo "</form>";
                                            echo "</div>";
                                        echo "</div>";
                                        echo "<div style='width: 425px; height: 25px;'></div>";
                                    echo "</div>";
                                    echo "<div style='width: 25px; height: 575px;'></div>";
                                }
                            }
                        ?>
                    </div>
            ...
            <section id="evidences" class="portfolio">
                <div class="container" data-aos="fade-up">
                    <div class="section-title"><h2>Evidences of solved cases</h2></div>
                        <div class='row' style='width: 900px; margin-left: 132.5px'>
                            <?php
                                $conn = new mysqli('localhost','root','','evidence_db');
                                if ($conn->connect_errno)
